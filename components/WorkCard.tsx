...
function WorkCard({ workItem }: Props): JSX.Element {
    ...
    return (
        <div id={"jobNo-" + jobNo} className='flex flex-col w-100 p-8 lg:p-16 rounded-2xl bg-slate-500 bg-opacity-10 mb-5 h-1/6'>
                ...
                <div className='mt-auto pt-10'>
                    {tagList.map(tag => <span key={`tag-${tag}`} className='text-sm text-slate-500 mr-5'>#{tag}</span>)}
                </div>
