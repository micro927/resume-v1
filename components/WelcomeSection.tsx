...
function WelcomeSection({ aboutMe, personalInformation }: WelcomeProps) {
    ...
    return (
        <section id='welcome' className='flex flex-col w-full '>
            <div className="flex flex-col md:flex-row w-100 min-h-screen md:min-h-max justify-between items-center">
                <div className='flex flex-col justify-center h-1/2 md:h-auto md:w-3/5 p-0 md:p-2 text-center md:text-left'>
                    ...
                    <div className='flex space-x-6 mt-3 justify-center md:justify-start'>
                        <div className='border rounded-lg p-1 bg-white text-2xl text-slate-900'><Link target={"_blank"} href={github}><FaGithub /></Link> </div>
                        <div className='border rounded-lg p-1 bg-white text-2xl text-slate-900'><Link target={"_blank"} href={linkedin}><FaLinkedinIn /></Link> </div>
                        <div className='border rounded-lg p-1 bg-white text-2xl text-slate-900'><Link target={"_blank"} href={"mailto:" + email}><FaEnvelope /></Link> </div>
                        <div className='border rounded-lg p-1 bg-white text-2xl text-slate-900'><Link target={"_blank"} href={facebook}><FaFacebookF /></Link> </div>
                    </div>
            ...
            <div className='p-8 lg:p-16 rounded-2xl bg-slate-500 bg-opacity-10'>
                <div>
                    ...
                    <div className="flex flex-col lg:flex-row w-100 h-full md:justify-between items-start text-lg mt-10">
                        <div className='flex flex-col text-sm lg:text-base justify-center w-full lg:w-1/2 xl:w-2/5 px-4 lg:px-9 order-last lg:order-first mt-5 lg:mt-0'>
                            <p className='indent-12 text-justify capitalize italic lg:not-italic'>
                                “ {aboutMe} ”
                            </p>
                        </div>
                        <div className='flex flex-col justify-center w-full lg:w-1/2 xl:w-2/5 px-4 text-sm sm:text-base'>
